...
# ====================== NLTK SETUP ======================
def setup_nltk():
    resources = ['punkt', 'wordnet', 'omw-1.4', 'averaged_perceptron_tagger', 
                 'maxent_ne_chunker', 'words', 'stopwords', 'punkt_tab', 'averaged_perceptron_tagger_eng']
    for res in resources:
        try:
...
def process_everything(text_input, file_input, num_sentences):
    # Fix for Gradio 6.0: Ensure slider value is an integer
    ...
    <div style='display: flex; justify-content: space-around; text-align: center; background: #f8f9fa; padding: 20px; border-radius: 10px; border: 1px solid #ddd;'>
        ...
        <div><small style='color: #666;'>COMPLEX WORDS</small><br><b style='font-size: 1.5em; color: #2c3e50;'>{textstat.difficult_words(text)}</b></div>
    </div>
    """
    ...
    return (
        ...
        kw_df,
        pd.DataFrame(pos_tags, columns=["Token", "POS Tag"]),
        json.dumps(blob.sentiment_assessments, indent=2)
    )

# ====================== GRADIO INTERFACE ======================
# Note: Theme moved to launch() to satisfy Gradio 6 requirements
with gr.Blocks(title="NLP PlayGround") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            file_in = gr.File(label="Upload Source File")
        with gr.Column(scale=2):
            gr.Markdown("### ⚙️ Settings")
            sum_slider = gr.Slider(1, 10, value=3, step=1, label="Summary Depth")
...
if __name__ == "__main__":
    # Theme passed here for Gradio 6 compatibility
